...
async function myCalendars() {
  ...
  async function open(f) {
    ...
    $("#folder-detail").innerHTML =
      `<h2>${esc(f.name)}</h2><label>Folder name<input id="folder-name" maxlength="100" value="${esc(f.name)}"></label><label>Subscription link<input id="folder-feed" readonly value="${esc(url)}"></label><div class="row"><button type="button" id="copy-folder">Copy subscription link</button><a class="button quiet" href="${esc(url)}">Download ICS</a></div><p class="hint">This link always includes the latest approved versions of your saved sources. Calendar apps refresh on their own schedule. Deadlines appear as labelled calendar markers at their due time, or all-day for date-only deadlines. Anyone with this private link can read the folder; resetting it disables the old link.</p>${incoming && !chosen.has(incoming) ? '<button id="add-incoming" class="quiet">Select the calendar you brought here</button>' : ""}<h3>Selected sources <span id="folder-count"></span></h3><div id="selected-sources"></div><details class="folder-browser"><summary>Add more calendars</summary><label>Find calendars<input id="folder-search" type="search" placeholder="Search events, locations, or hashtags"></label>${searchTools("folder")}<div class="source-results" id="folder-results"></div></details><p id="folder-message" role="status"></p><button id="save-folder">Save folder</button><p class="hint">Save source selections to update your subscription. The preview below shows saved selections.</p><div id="folder-preview"></div><div class="row"><button id="reset-folder" class="quiet">Reset subscription link</button><button id="delete-folder" class="danger">Delete folder</button></div>`;
    const selected = () => {
