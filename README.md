...
Approval is one SQLite transaction. Pending/rejected proposals do not affect feeds. Updates can be merged into the latest publication or explicitly overwrite it after a comparison preview. Three-way merges preserve independent changes and combine hashtag additions/removals. Conflicting edits, edit/delete conflicts, and competing additions with the same UID require a manager choice. Recurring series and their overrides are resolved together, preventing partial series deletion. Calendar metadata conflicts are also explicit. Publication checks the previewed revision inside the transaction, so another approval requires a fresh comparison. A rejected contributor can reload the current calendar and submit a new proposal. Drafts stay in the current page until submission; leaving the editor prompts before discarding unsent changes.
...
The editor has a create/proposal mode switch, a month preview, and three entry types: events (start and end), deadlines (due date only), and notices (start only). Neither new-calendar submissions nor update proposals require a change explanation. Imports live beside the entry tools: upload an ICS file, search and select multiple published calendars, or paste a public HTTP/HTTPS/webcal subscription link. Link imports add a current snapshot to the draft; they do not automatically publish future provider updates. Imported URLs are not saved in public metadata. Downloads are limited to 5 MB and public network destinations, including redirects. A source must be explicitly loaded before submitting an update using the same searchable browser, with a single target selection. “Create draft from selected calendars” merges selected sources into a new draft for manager review.

Repeat controls support daily, weekly, monthly and yearly schedules, intervals, clearly selected weekday buttons (starting with the entry’s weekday), end dates and occurrence counts. Imported rules are shown in ordinary words and preserved unless the contributor changes them. Separate date and time fields are available for every entry type; all-day entries have an explicit switch. Crossing start/end inputs move the opposite endpoint to preserve the previous duration. A timezone dropdown retains imported/custom zones. Date/time controls retain calendar timezones; all-day end dates are exclusive. The month preview expands recurring entries, exclusions and occurrence overrides, including daylight-saving transitions. Preview expansion is bounded to six weeks and 2,000 displayed occurrences, with a visible warning for unsupported or truncated rules. It does not offer drag-and-drop editing.
...
### U of T Engineering section export

Run `python tools/export_utoronto.py --output /path/to/new-export` in the project environment to export the public Timetable Builder APSC offerings for Fall 2026, Winter 2027, and full-year 2026–2027. Each section gets a separate ICS and complete JSON record, with an `index.csv` recording event counts and warnings. Use `--cached` with an existing export to regenerate from its saved API pages.

The script uses published Engineering session dates and excludes holidays and study breaks. Alternating-week, manual, and TBA schedules without reliable dates are preserved as metadata-only files, without inventing occurrences. Public building information is retained; private ACORN room details are unavailable. Review the term/date constants before adapting the script to another year or faculty. Use a fresh output directory for each new snapshot.
